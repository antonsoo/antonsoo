...
// embedded base64 JPEG (768px Thoth panel, regenerated by scripts/make-thoth.mjs).
...
const FRESCO = readFileSync(join(__dirname, 'thoth.b64'), 'utf8').trim();
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" role="img" aria-label="Anton Soloviev. Founder and CEO of PRAVIEL. Reviving the languages the world calls dead.">
  ...
  <g opacity="1"><animate attributeName="opacity" from="0" to="1" dur="1s" begin="0s" fill="freeze"/>
    ...
    <image href="${FRESCO}" x="${PX}" y="${PY}" width="${PS}" height="${PS}" clip-path="url(#panelClip)" preserveAspectRatio="xMidYMid slice"/>
